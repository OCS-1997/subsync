...
CREATE TABLE IF NOT EXISTS knowledge_articles (
    ...
    -- SEO Fields
    meta_title VARCHAR(60) DEFAULT NULL COMMENT 'SEO meta title (max 60 chars)',
    meta_description VARCHAR(160) DEFAULT NULL COMMENT 'SEO meta description (max 160 chars)',
    keywords TEXT DEFAULT NULL COMMENT 'SEO keywords (comma-separated)',
    canonical_url VARCHAR(255) DEFAULT NULL COMMENT 'Canonical URL for SEO',
    og_image VARCHAR(500) DEFAULT NULL COMMENT 'Open Graph image URL',
    
    -- Analytics Fields
    total_reads INT DEFAULT 0 COMMENT 'Total read count',
    unique_reads INT DEFAULT 0 COMMENT 'Unique visitor count',
    last_read_at TIMESTAMP NULL DEFAULT NULL COMMENT 'Last time article was read',
    ...
    KEY idx_public_articles (visibility, is_published, slug),
    KEY idx_sitemap (visibility, is_published, updated_at),
    FULLTEXT KEY ft_search (title, content),
    FOREIGN KEY (category_id) REFERENCES knowledge_categories(id) ON DELETE SET NULL,
    FOREIGN KEY (author_id) REFERENCES users(username) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
-- Knowledge Base Article-Tag Mapping
...
-- Knowledge Base Sources (DCR mapping)
CREATE TABLE IF NOT EXISTS knowledge_sources (
    id INT NOT NULL AUTO_INCREMENT,
    article_id INT NOT NULL,
    source_type ENUM('dcr','manual','import') NOT NULL DEFAULT 'manual',
    source_reference_id INT DEFAULT NULL COMMENT 'DCR entry ID if source is DCR',
    created_at TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
    KEY idx_article_id (article_id),
    KEY idx_source_reference (source_reference_id),
    FOREIGN KEY (article_id) REFERENCES knowledge_articles(id) ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
...
CREATE TABLE IF NOT EXISTS knowledge_versions (
    ...
    PRIMARY KEY (id),
    KEY idx_article_id (article_id),
    KEY idx_version_number (article_id, version_number),
    FOREIGN KEY (article_id) REFERENCES knowledge_articles(id) ON DELETE CASCADE,
    FOREIGN KEY (changed_by) REFERENCES users(username) ON DELETE RESTRICT
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;

-- =============================================
-- KNOWLEDGE BASE SECURITY & ANALYTICS
-- =============================================
...
-- Add KB permissions
INSERT INTO permissions (permission_key, resource, action, description) VALUES
('knowledge_base.view', 'knowledge_base', 'view', 'View knowledge base articles'),
('knowledge_base.create', 'knowledge_base', 'create', 'Create knowledge base articles'),
('knowledge_base.update', 'knowledge_base', 'update', 'Update knowledge base articles'),
('knowledge_base.delete', 'knowledge_base', 'delete', 'Delete knowledge base articles'),
('knowledge_base.manage_categories', 'knowledge_base', 'manage_categories', 'Manage knowledge base categories')
ON DUPLICATE KEY UPDATE description = VALUES(description);

-- Assign KB permissions to roles
INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id
FROM roles r
JOIN permissions p ON p.permission_key IN (
    ...
    'knowledge_base.manage_categories'
)
...
WHERE r.role_key IN ('support', 'viewer')
ON DUPLICATE KEY UPDATE role_id = VALUES(role_id);

-- =============================================
-- KNOWLEDGE BASE CLEANUP EVENTS
-- =============================================
