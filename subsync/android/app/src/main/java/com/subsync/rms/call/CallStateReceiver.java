...
/**
 * CallStateReceiver — BroadcastReceiver that tracks phone call state
 * transitions.
 *
 * State machine:
 ...
 *
 * Fires the listener with phone number, duration, and call type when a call
 * ends.
 */
public class CallStateReceiver extends BroadcastReceiver {
    ...
    private static CallEndListener staticListener;

    // State tracking (static so it survives onReceive calls which create new
    // instances)
    ...
    public CallStateReceiver() {
        // No-arg constructor for Manifest
        this.listener = null;
    }
    ...
    private CallEndListener getListener() {
        return (this.listener != null) ? this.listener : staticListener;
    }
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (TelephonyManager.EXTRA_STATE_RINGING.equals(state)) {
            ...
            if (TelephonyManager.EXTRA_STATE_OFFHOOK.equals(lastState)) {
                ...
                // Persistent storage for app-closed scenarios
                Context ctx = context.getApplicationContext();
                ctx.getSharedPreferences("CallDetector", Context.MODE_PRIVATE)
                        .edit()
                        .putString("pending_number", finalPhone)
                        .putInt("pending_duration", finalDur)
                        .putString("pending_type", finalType)
                        .putBoolean("is_pending", true)
                        .apply();

                CallEndListener activeListener = getListener();
                if (activeListener != null) {
                    activeListener.onCallEnded(finalPhone, finalDur, finalType);
                } else {
                    // Fallback to static bridge in Plugin
                    CallDetectorPlugin.onCallDetectedStatic(finalPhone, finalDur, finalType);
                }
                ...
                context.getApplicationContext().getSharedPreferences("CallDetector", Context.MODE_PRIVATE)
                        .edit()
                        .putString("pending_number", finalPhone)
                        .putInt("pending_duration", 0)
                        .putString("pending_type", "missed")
                        .putBoolean("is_pending", true)
                        .apply();

                CallEndListener activeListener = getListener();
                if (activeListener != null) {
                    activeListener.onCallEnded(finalPhone, 0, "missed");
                } else {
                    CallDetectorPlugin.onCallDetectedStatic(finalPhone, 0, "missed");
                }
            }
